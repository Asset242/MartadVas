        ...
        <div class="table-responsive text-nowrap">
            <table class="table">
                ...
                <tbody class="table-border-bottom-0">
                  @foreach ($products as $index => $product) <!-- Added $index to get the auto-increment number -->
                  <tr>
                      <td>{{ $index + 1 }}</td> <!-- Auto-increment serial number (starts from 1) -->

                      <td>{{ $product->plan_id }}</td>
                      <td>{{ $product->duration}}</td>
                      <td>{{ $product->amount}}</td>
                      <td>{{ $product->coin}}</td>
                      <td>{{ $product->user->name}}</td>

                      <td>{{ \Carbon\Carbon::parse($product->created_at)->format('d M Y, H:i') }}</td> <!-- Formatted date -->
                      ...
                      <td>
                          <div class="dropdown">
                              ...
                              <div class="dropdown-menu">
                                  <!-- Edit Post -->
                                  <a class="dropdown-item" href="{{ route('edit', $product->id) }}">
                                      <i class="bx bx-edit-alt me-1"></i> Edit
                                  </a>
                                  <!-- Delete Post -->
                                  <form action="{{ route('posts.destroy', $product->id) }}" method="POST" style="display: inline;">
                                      @csrf
                                      @method('DELETE')
